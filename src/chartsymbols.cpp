...
static wxArrayPtrVoid * colorTables = NULL; // FIXME: use std containers
...
static symbolGraphicsHashMap * symbolGraphicLocations = NULL;
//--------------------------------------------------------------------------------------
...
{
	for( TiXmlNode *childNode = colortableNodes->FirstChild(); childNode; childNode = childNode->NextSibling() ) {
...
int ChartSymbols::LoadRasterFileForColorTable( int tableNo )
{
	...
	wxLogMessage(wxString(_T("ChartSymbols...Failed to load raster symbols file ")) + filename);
	return false;
}
