...
bool TCMgr::GetTideOrCurrent15(time_t WXUNUSED(t), int idx, float& tcvalue, float& dir,
							   bool& bnew_val)
{
	int ret = 0;
	IDX_entry* pIDX = m_Combined_IDX_array.at(idx); // point to the index entry
...
void TCMgr::GetHightOrLowTide(time_t t, int sch_step_1, int sch_step_2, float tide_val, bool w_t,
...
#define stretchfactor 3
		...
		HI = BOGUS_amplitude(HI, pIDX) + pIDX->pref_sta_data->DATUM;
		{
			...
			HS = HI * ((RH + RL) / 2 + (RH - RL) * magicnum) + (HH + HL) / 2 + (HH - HL) * magicnum;
		}
...
static double time2dt_tide(time_t t, int deriv, IDX_entry* pIDX)
{
	...
	if (t - s_this_epoch <= TIDE_BLEND_TIME && s_this_year > pIDX->first_year)
		return blend_tide(t, deriv, s_this_year - 1, (double)(t - s_this_epoch) / TIDE_BLEND_TIME,
						  pIDX);
	else if (s_next_epoch - t <= TIDE_BLEND_TIME && s_this_year + 1 < pIDX->first_year
																	  + pIDX->num_epochs)
		return blend_tide(t, deriv, s_this_year, -(double)(s_next_epoch - t) / TIDE_BLEND_TIME,
						  pIDX);
