		...
		dsPortType io_select)
{
	...
	for(int i=0 ; i  < OUT_QUEUE_LENGTH ; i++){
		// FIXME: memory leak: m_poutQueue[*] is allocated but not released
		m_poutQueue[i] = static_cast<char*>(malloc(MAX_OUT_QUEUE_MESSAGE_LENGTH));
	}
	m_takIndex = -1;
	m_putIndex = -1;
			...
			if (ReadFile((HANDLE)m_gps_fd, &chRead, 1, &dwOneRead, NULL)) {
				if (1 == dwOneRead) {
					...
					if (n_timeout > max_timeout) {
						...
						//  If nothing has been input from the port for a long time (10 sec), it may
						// be broken.
						...
						//  Some virtual port emulators (XPort, especially) do not honor
						// COMMTIMEOUTS
						//  So, we must check the real time (in seconds) of the inactivity time.
						//  If it is much shorter than 10 secs, assume that COMMTIMEOUTS are not
						// working
						//  In this case, abort the port reset, reset the counters,
						//  and signal a need for a Sleep() between read commands to avoid CPU
						// saturation.
...
void OCP_DataStreamInput_Thread::ThreadMessage(const wxString& msg)
{
	...
	event.SetEventObject(reinterpret_cast<wxObject*>(this));
	event.SetString(msg);
	if (gFrame)
...
bool OCP_DataStreamInput_Thread::SetOutMsg(const wxString& msg)
{
	...
	if ((m_takIndex == 0 && m_putIndex == OUT_QUEUE_LENGTH - 1)
		|| (m_takIndex == (m_putIndex + 1))) {
		return false;
	} else {
...
{
	ssize_t status = write(port_descriptor, string.mb_str(), string.Len());
	return status;
}

int OCP_DataStreamInput_Thread::WriteComPortPhysical(int port_descriptor, char* msg)
{
	ssize_t status = write(port_descriptor, msg, strlen(msg));
	return status;
}

int OCP_DataStreamInput_Thread::ReadComPortPhysical(int port_descriptor, int count,
													unsigned char* p)
{
	// Blocking, timeout protected read of one character at a time
	// Timeout value is set by c_cc[VTIME]
