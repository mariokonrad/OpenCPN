...
void ChartCanvas::ShipDraw( ocpnDC& dc )
{
	...
	geo::ll_gc_ll(nav.lat, nav.lon, pCog, pSog * g_ownship_predictor_minutes / 60., &pred_lat, &pred_lon );
	...
	geo::ll_gc_ll(nav.lat, nav.lon, icon_hdt, pSog * 10. / 60., &osd_head_lat, &osd_head_lon);
	...
	geo::ll_gc_ll(nav.lat, nav.lon, icon_hdt, pSog * g_ownship_predictor_minutes / 60., &hdg_pred_lat, &hdg_pred_lon);
	...
	if (GetVP().GetBBox().LineIntersect(wxPoint2DDouble(nav.lon, nav.lat), wxPoint2DDouble(pred_lon, pred_lat)))
		drawit++;
	...
	if( drawit ) {
		...
		if( GetVP().chart_scale > 300000 )             // According to S52, this should be 50,000
		{
			...
			if( g_n_ownship_beam_meters > 0.0 && g_n_ownship_length_meters > 0.0 && g_OwnShipIconType > 0 ) // use large ship
			{
				...
				geo::ll_gc_ll(nav.lat, nav.lon, icon_hdt, g_n_ownship_length_meters / 1852., &ship_bow_lat, &ship_bow_lon );
	...
	if( drawit ) {
		...
		if( ((td->n_alarm_state == AIS_ALARM_SET) && (td->bCPA_Valid)) || (td->b_show_AIS_CPA && (td->bCPA_Valid))) {
			...
			if (wxIsNaN(nav.cog) || wxIsNaN(nav.sog)) {
				...
				geo::ll_gc_ll(nav.lat, nav.lon, nav.cog, nav.sog * td->TCPA / 60.0, &ocpa_lat, &ocpa_lon);
			}
...
void ChartCanvas::PopupMenuHandler( wxCommandEvent& event )
{
	...
	switch( event.GetId() ) {
		...
		case ID_DEF_MENU_GOTO_HERE: {
			...
			pSelect->AddSelectableRouteSegment(nav.lat, nav.lon, zlat, zlon, pWP_src, pWP_dest, temp_route);
		...
		case ID_RT_MENU_ACTIVATE: {
									...
									if( g_pRouteMan->GetpActiveRoute() )
									  ...
									  RoutePoint *best_point = g_pRouteMan->FindBestActivatePoint(
											m_pSelectedRoute, nav.lat, nav.lon, nav.cog, nav.sog);
...
void ChartCanvas::RenderChartOutline( ocpnDC &dc, int dbIndex, ViewPort& vp )
{
	...
	if (vp.GetBBox().Intersect( box, 0 ) != geo::BoundingBox::_OUT) // chart is not outside of viewport
		b_draw = true;
...
void ChartCanvas::DrawAllRoutesInBBox( ocpnDC& dc, geo::LatLonBoundingBox & BltBBox, const wxRegion& clipregion )
{
	...
	for (RouteList::iterator i = pRouteList->begin(); i != pRouteList->end(); ++i) {
		...
		if( pRouteDraw ) {
			...
			if( BltBBox.Intersect( test_box, 0 ) != geo::BoundingBox::_OUT ) // Route is not wholly outside window
			{
				b_drawn = true;
				...
				if (BltBBox.Intersect(test_box1, 0) != geo::BoundingBox::_OUT) // Route is not wholly outside window
				{
					b_drawn = true;
					if ((pRouteDraw != active_route) && (pRouteDraw != active_track))
						pRouteDraw->Draw(dc, GetVP());
			...
			if( !b_drawn ) {
				if( ( BltBBox.GetMinX() < -180. ) && ( BltBBox.GetMaxX() > -180. ) ) {
					...
					if (BltBBox.Intersect(test_box2, 0) != geo::BoundingBox::_OUT) // Route is not wholly outside window
					{
						b_drawn = true;
						if ((pRouteDraw != active_route) && (pRouteDraw != active_track))
							pRouteDraw->Draw(dc, GetVP());
					...
					if (BltBBox.Intersect( test_box3, 0 ) != geo::BoundingBox::_OUT) // Route is not wholly outside window
					{
						b_drawn = true;
						if ((pRouteDraw != active_route) && (pRouteDraw != active_track))
							pRouteDraw->Draw(dc, GetVP());
...
void ChartCanvas::DrawAllCurrentsInBBox( ocpnDC& dc, geo::LatLonBoundingBox & BBox, bool bRebuildSelList,
