...
#define CM93_ZOOM_FACTOR_MAX_RANGE 5 // FIXME: better solution (maybe over global infrastructure)
			...
			case 20: // Ctrl T
				if (NULL == pGoToPositionDialog) // There is one global instance of the Go To
												 // Position Dialog
...
void ChartCanvas::OnRolloverPopupTimerEvent(wxTimerEvent&)
{
	...
	if (g_pAIS && g_pAIS->GetNumTargets() && g_bShowAIS) {
		SelectItem* pFind
			= pSelectAIS->FindSelection(m_cursor_lat, m_cursor_lon, SelectItem::TYPE_AISTARGET);
		if (pFind) {
			int FoundAIS_MMSI
	...
	if (NULL == m_pRolloverRouteSeg) {
		...
		SelectableItemList SelList
			= pSelect->FindSelectionList(m_cursor_lat, m_cursor_lon, SelectItem::TYPE_ROUTESEGMENT);
...
void ChartCanvas::OnCursorTrackTimerEvent(wxTimerEvent&)
{
	...
	if ((fabs(cursor_lat) < 90.0) && (fabs(cursor_lon) < 360.0)) {
		while (cursor_lon < -180.0)
			cursor_lon += 360.0;
...
void ChartCanvas::ShipDraw(ocpnDC& dc)
{
	...
	geo::ll_gc_ll(nav.lat, nav.lon, pCog, pSog * g_ownship_predictor_minutes / 60., &pred_lat,
				  &pred_lon);

	GetCanvasPointPix(nav.lat, nav.lon, &lGPSPoint);
	...
	GetCanvasPointPix(nav.lat, nav.lon, &lShipMidPoint);
...
void ChartCanvas::AISDrawTarget(ais::AIS_Target_Data* td, ocpnDC& dc)
{
	...
	if (drawit) {
		...
		if (((td->n_alarm_state == AIS_ALARM_SET) && (td->bCPA_Valid))
			...
			wxPoint oShipPoint;
			GetCanvasPointPix(nav.lat, nav.lon, &oShipPoint);
		...
		if (g_bAISShowTracks && td->b_show_track) {
			...
			if (track_point != td->m_ptrack.end()) {
				GetCanvasPointPix(track_point->m_lat, track_point->m_lon, &TrackPointA);
				++track_point;
			}
...
void ChartCanvas::FindRoutePointsAtCursor(float, bool setBeingEdited)
{
	...
	SelectableItemList SelList
		= pSelect->FindSelectionList(m_cursor_lat, m_cursor_lon, SelectItem::TYPE_ROUTEPOINT);
...
void ChartCanvas::mouse_left_center(const wxMouseEvent& event)
{
	...
	double zlat;
	double zlon;
	GetCanvasPixPoint(event.GetX(), event.GetY(), zlat, zlon);

	SelectItem* pFindAIS = pSelectAIS->FindSelection(zlat, zlon, SelectItem::TYPE_AISTARGET);
	...
	cursorItem = pSelect->FindSelection(zlat, zlon, SelectItem::TYPE_ROUTESEGMENT);
	...
	cursorItem = pSelect->FindSelection(zlat, zlon, SelectItem::TYPE_TRACKSEGMENT);
	...
	ShowObjectQueryWindow(event.GetX(), event.GetY(), zlat, zlon);
}
...
void ChartCanvas::MouseEvent(wxMouseEvent & event)
{
	...
	if (event.RightDown()) {
		...
		if (parent_frame->nRoute_State) { // creating route?
			...
			pFindAIS = pSelectAIS->FindSelection(slat, slon, SelectItem::TYPE_AISTARGET);
			pFindRP = pSelect->FindSelection(slat, slon, SelectItem::TYPE_ROUTEPOINT);
			pFindRouteSeg = pSelect->FindSelection(slat, slon, SelectItem::TYPE_ROUTESEGMENT);
			pFindTrackSeg = pSelect->FindSelection(slat, slon, SelectItem::TYPE_TRACKSEGMENT);

			if (m_bShowCurrent)
				pFindCurrent = pSelectTC->FindSelection(slat, slon, SelectItem::TYPE_CURRENTPOINT);

			if (m_bShowTide) // look for tide stations
				pFindTide = pSelectTC->FindSelection(slat, slon, SelectItem::TYPE_TIDEPOINT);
			...
			if (pFindRP) {
				...
				SelectableItemList SelList
					= pSelect->FindSelectionList(slat, slon, SelectItem::TYPE_ROUTEPOINT);
				for (SelectableItemList::iterator index = SelList.begin(); index != SelList.end(); ++index) {
					...
					bool brp_viz = false;
					if (proute_array) {
						for (Routeman::RouteArray::iterator i = proute_array->begin();
							 i != proute_array->end(); ++i) {
			...
			if (pFindRouteSeg) { // there is at least one select item
				SelectableItemList SelList
					= pSelect->FindSelectionList(slat, slon, SelectItem::TYPE_ROUTESEGMENT);
			...
			if (pFindTrackSeg) {
				m_pSelectedTrack = NULL;
				SelectableItemList SelList
					= pSelect->FindSelectionList(slat, slon, SelectItem::TYPE_TRACKSEGMENT);
			...
			if (pFindCurrent) {
				...
				SelectableItemList SelList = pSelectTC->FindSelectionList(
					m_cursor_lat, m_cursor_lon, SelectItem::TYPE_CURRENTPOINT);
...
void ChartCanvas::PopupMenuHandler(wxCommandEvent& event)
{
	...
	switch (event.GetId()) {
		...
		case ID_DEF_MENU_MOVE_BOAT_HERE:
			global::OCPN::get().nav().set_latitude(zlat);
			global::OCPN::get().nav().set_longitude(zlon);
			break;
		...
		case ID_WP_MENU_DELPOINT: {
			...
			if (m_pFoundRoutePoint && !(m_pFoundRoutePoint->m_bIsInLayer)
				...
				Routeman::RouteArray* proute_array
