...
TTYWindow::TTYWindow(wxWindow *parent, int n_lines, WindowDestroyListener * listener)
	...
	, m_pScroll(NULL)
{
	wxDialog::Create(parent, -1, _T("Title"), wxDefaultPosition, wxDefaultSize,
	...
	m_buttonPause->Connect(wxEVT_COMMAND_BUTTON_CLICKED,
						   wxCommandEventHandler(TTYWindow::OnPauseClick), NULL, this);
...
void TTYWindow::CreateLegendBitmap()
{
	...
	if (m_bm_legend.IsOk()) {
		...
		dc.SetBackground(wxBrush(global::OCPN::get().color().get_color(_T("DILG1"))));
		dc.Clear();
