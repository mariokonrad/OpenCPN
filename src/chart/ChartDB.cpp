...
int ChartDB::BuildChartStack(ChartStack* cstk, float lat, float lon)
{
	...
	for (int db_index = 0; db_index < nEntry; db_index++) {
		...
		if (b_group_add) {
			...
			} else if ((pt->GetLonMax() > 180.) && (pt->GetLonMin() < 180.0)) {
				// Check the special case where chart spans the international dateline
				if (CheckPositionWithinChart(db_index, lat, lon + 360.0)
					&& (j < ChartStack::MAXSTACK)) {
					j++;
					cstk->nEntry = j;
					cstk->SetDBIndex(j - 1, db_index);
				}
			} else if ((pt->GetLonMax() > 180.0) && (pt->GetLonMin() > 180.0)) {
	...
	// Remove exact duplicates, i.e. charts that have exactly the same file name and nearly the same
	// mod time
	// These charts can be in the database due to having the exact same chart in different
	// directories,
...
bool ChartDB::CheckPositionWithinChart(int index, float lat, float lon)
{
	...
	if ((lat <= pt->GetLatMax()) && (lat >= pt->GetLatMin()) && (lon >= pt->GetLonMin())
		&& (lon <= pt->GetLonMax())) {
		// Double check on Primary Ply points polygon

		bool bInside
			= G_FloatPtInPolygon((MyFlPoint*)pt->GetpPlyTable(), pt->GetnPlyEntries(), lon, lat);
	...
	if (!bInCache) { // not in cache
		...
		if (sys.memCacheLimit) {
			...
			while ((mem_used > sys.memCacheLimit * 8 / 10) && !m_b_locked
				...
				if (nCache > 2) {
					...
					int iOldest = 0;
					for (unsigned int i = 0; i < nCache; i++) {
						pce = static_cast<CacheEntry*>(pChartCache->Item(i));
					...
					pce = static_cast<CacheEntry*>(pChartCache->Item(iOldest));
		...
		if (Ch) {
			...
			if (INIT_OK == ir) {
				...
				// run, but will stay in the database
				DisableChart(ChartFullPath);
			} else if ((INIT_FAIL_RETRY == ir) || (INIT_FAIL_NOERROR == ir)) {
				// recoverable problem in chart Init()
				delete Ch;
				Ch = NULL;
			}
...
bool G_FloatPtInPolygon(MyFlPoint *rgpts, int wnumpts, float x, float y)
{
	...
	// If result is false, check the degenerate case where test point lies on a polygon
	// endpoint
	if (!(wnumintsct & 1)) {
