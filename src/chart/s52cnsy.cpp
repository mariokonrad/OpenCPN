	...
	if (0 != quapos) {
		if (2 <= quapos && quapos < 10) {
			if (safe) {
				...
				S57Obj tempObj;
				LUPrec* safelup
					= ps52plib->S52_LUPLookup(PLAIN_BOUNDARIES, "SAFECD", &tempObj, false);
				if (safelup)
	...
	if (NULL != restrnstr) {
		...
		if (strpbrk(restrn, "\007\010\016")) { // entry restrictions
			...
			if (strpbrk(restrn, "\001\002")) { // anchoring
				...
				if (TRUE == S52_getMarinerParam(S52_MAR_SYMBOLIZED_BND))
					line = _T(";LC(RESARE51)"); // could be ACHRES51 when _drawLC is implemented
												// fully
				else
					line = _T(";LS(DASH,2,CHMGD)");
...
static wxString SNDFRM02(S57Obj* obj, double depth_value_in)
{
	...
	if (depth_value < 10000.0) {
		...
		double third_digit
			= floor((leading_digit - (first_digit * 1000) - (secnd_digit * 100)) / 10);
		double last_digit = floor(leading_digit - (first_digit * 1000) - (secnd_digit * 100)
								  - (third_digit * 10));
	...
	// Continuation C
	{
		...
		double third_digit
			= floor((leading_digit - (first_digit * 10000) - (secnd_digit * 1000)) / 100);
		double fourth_digit = floor((leading_digit - (first_digit * 10000) - (secnd_digit * 1000)
									 - (third_digit * 100)) / 10);
		double last_digit = floor(leading_digit - (first_digit * 10000) - (secnd_digit * 1000)
...
static void* WRECKS02(void* param)
{
	...
	if (GEO_POINT == obj->Primitive_type) {
		if (0 != udwhaz03str->Len()) {
			...
			if (UNKNOWN != valsou) {
				if ((valsou < safety_contour) /* || (2 == catwrk)*/) // maybe redundant, seems like
																	 // wrecks with valsou < 20
