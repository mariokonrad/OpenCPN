...
void SendToGpsDlg::CreateControls(const wxString& WXUNUSED(hint))
{
	...
	for (unsigned int iPortIndex = 0; iPortIndex < pSerialArray->size(); iPortIndex++) {
	...
	wxStaticText* premtext
		= new wxStaticText(this, -1, _("Prepare GPS for Route/Waypoint upload and press Send..."));
	...
	m_SendButton
		= new wxButton(itemDialog1, ID_STG_OK, _("Send"), wxDefaultPosition, wxDefaultSize, 0);
