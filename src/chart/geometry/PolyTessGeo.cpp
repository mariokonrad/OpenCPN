...
bool isRingClockwise(wxPoint2DDouble* pp, int nPointCount)
{
	...
	for (int iVert = 0; iVert < nPointCount - 1; iVert++) {
		dfSum += pp[iVert].m_x * pp[iVert + 1].m_y - pp[iVert].m_y * pp[iVert + 1].m_x;
	}

	dfSum += pp[nPointCount - 1].m_x * pp[0].m_y - pp[nPointCount - 1].m_y * pp[0].m_x;
...
int PolyTessGeo::BuildTessTri(void)
{
	...
	bool bSENC_SM = false;
	const geo::Position reference_point(0.0, 0.0);
	...
	for (int ip = 1; ip < npta + 1; ip++) {
		...
		if (bSENC_SM) {
			// Calculate SM from chart common reference point
			double easting, northing;
			geo::toSM(geo::Position(ty, tx), reference_point, &easting, &northing);
			*vro++ = easting; // x
			*vro++ = northing; // y
		} else {
			*vro++ = tx; // lon
		...
		if (pr->is_valid) {
			...
			if (bSENC_SM) {
				for (int i = 0; i < pr->nvert; i++) {
					...
					double easting, northing;
					geo::toSM(geo::Position(dlat, dlon), reference_point, &easting, &northing);
					*pdd++ = easting;
					*pdd++ = northing;
				}
			} else {
