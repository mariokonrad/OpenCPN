		...
		if (m_portstring.Contains(_T("Serial"))) {
			...
			if ((wxNOT_FOUND != port_uc.Find(_T("USB")))
				...
				if (hSerialComm == INVALID_HANDLE_VALUE) {
					...
					wxLogMessage(_T("   Error, comm port open failure, INVALID_HANDLE_VALUE, ")
								 _T("Datastream skipped."));
					return;
				} else {
				...
				m_pSecondary_Thread = new OCP_DataStreamInput_Thread(
					this, m_consumer, comx, m_BaudRate, &m_output_mutex, m_io_select);
				m_Thread_run_flag = 1;
		...
		if (m_connection_type == ConnectionParams::NETWORK) {
			...
			switch (m_net_protocol) {
				...
				case ConnectionParams::TCP:
					...
					if ((m_io_select == DS_TYPE_INPUT_OUTPUT) || (m_io_select == DS_TYPE_OUTPUT)) {
						...
						if (m_socket_server->IsOk()) {
							int nagleDisable = 1;
							m_socket_server->SetOption(IPPROTO_TCP, TCP_NODELAY, &nagleDisable,
													   sizeof(nagleDisable));
						}

						// Drastically reduce the size of the socket output buffer
						// so that when client goes away without properly closing, the stream
						// will quickly fill the output buffer, and thus fail the write() call
						// within a few seconds.
						unsigned long outbuf_size = 512;
						if (m_socket_server->IsOk()) {
							m_socket_server->SetOption(SOL_SOCKET, SO_SNDBUF, &outbuf_size,
													   sizeof(outbuf_size));
						}
				...
				case ConnectionParams::UDP:
					...
					in_addr_t addr = reinterpret_cast<const struct sockaddr_in*>(
						m_addr.GetAddressData())->sin_addr.s_addr;
#else
					in_addr_t addr = reinterpret_cast<const struct sockaddr_in*>(
						m_addr.GetAddress()->m_addr)->sin_addr.s_addr;
#endif
#else
					...
					if ((m_io_select == DS_TYPE_INPUT_OUTPUT) || (m_io_select == DS_TYPE_OUTPUT)) {
						...
						m_tsock = new wxDatagramSocket(tconn_addr,
													   wxSOCKET_NOWAIT | wxSOCKET_REUSEADDR);
...
void DataStream::OnSocketReadWatchdogTimer(wxTimerEvent&)
{
	...
	if (m_dog_value <= 0) { // No receive in n seconds, assume connection lost
		wxLogMessage(
			wxString::Format(_T("    TCP Datastream watchdog timeout: %s"), m_portstring.c_str()));
...
{
// Allows handling of high volume data streams, such as a National AIS stream with 100s of msgs a second.
#define RD_BUF_SIZE 4096

	switch (event.GetSocketEvent()) {
		case wxSOCKET_INPUT: // from gpsd Daemon
		{
			// TODO determine if the follwing SetFlags needs to be done at every socket event or
			// only once when socket is created, it it needs to be done at all!
			...
			// Prevent non-nmea junk from consuming to much memory by limiting carry-over buffer
			// size.
		...
		case wxSOCKET_LOST: {
			if (m_net_protocol == ConnectionParams::TCP) {
				wxLogMessage(wxString::Format(_T("TCP Datastream connection lost: %s"),
											  m_portstring.c_str()));
				...
				// If the socket has never connected, and it is a short interval since the connect
				// request
				// then stretch the time a bit.  This happens on Windows if there is no dafault IP
				// on any interface
		...
		case wxSOCKET_CONNECTION: {
			if (m_net_protocol == ConnectionParams::GPSD) {
				...
				wxLogMessage(wxString::Format(_T("TCP Datastream connection established: %s"),
											  m_portstring.c_str()));
				m_dog_value = N_DOG_TIMEOUT; // feed the dog
...
bool DataStream::SentencePassesFilter(const wxString& sentence,
