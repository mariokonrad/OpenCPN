...
OCPN_NMEAEvent::OCPN_NMEAEvent(wxEventType commandType, int id) : wxEvent(id, commandType)
{
}
...
wxEvent* OCPN_NMEAEvent::Clone() const
{
	...
	newevent->m_NMEAstring
		= this->m_NMEAstring.c_str(); // this enforces a deep copy of the string data
	return newevent;
}
...
// FIXME: this is a clusterfuck of method length and preprocessor crap
NMEAHandler::NMEAHandler(int handler_id, wxFrame* frame, const wxString& NMEADataSource,
						 const wxString& strBaudRate, wxMutex* pMutex, bool bGarmin)
{
	...
	//    NMEA Data Source is specified serial port
		...
		m_pSecondary_Thread
			= new OCP_NMEA_Thread(this, frame, pMutex, m_pPortMutex, comx, g_pCommMan, strBaudRate);
		m_Thread_run_flag = 1;
...
#ifdef __POSIX__
		//    Kick off the NMEA RX thread
		m_pSecondary_Thread = new OCP_NMEA_Thread(this, frame, pMutex, m_pPortMutex, comx,
												  g_pCommMan, strBaudRate, true);
		m_Thread_run_flag = 1;
		m_pSecondary_Thread->Run();
#endif
...
#define DYNAMIC_LOAD_LIBGPS 1
		...
		if (!m_lib_handle) {
			...
			if ((0 == libgps_api) || !s_fn_gps_open || !s_fn_gps_close || !s_fn_gps_waiting
				|| !s_fn_gps_stream) {
		...
		if (19 == libgps_api) {
			...
			if (!gps_19data) {
				...
				OCPNMessageDialog md(m_parent_frame, msg, _("OpenCPN Message"),
			...
			result_open
				= s_fn_gps_open(NMEA_data_ip.char_str(), (char*)DEFAULT_GPSD_PORT, pgps_data);
		...
		if (!result_open) { // opened OK
			...
			check_version.proto_major = 0;
			check_version.proto_minor = 0;
			...
			if (b_version_set) {
				wxString msg;
				msg.Printf(_T("LIBGPS: Found libgps version %d.%d"), check_version.proto_major,
						   check_version.proto_minor);
				wxLogMessage(msg);
			} else
...
#define RD_BUF_SIZE 200
	...
	switch (event.GetSocketEvent()) {
			...
			*bp = 0; // end string
			...
			if (str_buf.StartsWith(_T("GPSD,O"))) // valid position data?
			{
				if (str_buf.GetChar(7) != '?') // GPSd connected and we get real data
				{
					...
					if (token.ToDouble(&dgsog)) {
						...
						if (m_bgot_version) {
							if ((m_gpsd_major >= 2)
								&& (m_gpsd_minor > 23)) // Speed data will be in meters/sec.
								ThreadPositionData.kSog = dgsog * 3600.0
														  / 1852.0; // convert from m/s to knots
						}
					}
			...
			} else if (str_buf.StartsWith(_T("GPSD,L"))) // version report?
			{
				...
				if (ver_token.ToDouble(&gpsd_ver)) {
					...
					m_gpsd_minor
						= (int)floor(((gpsd_ver - m_gpsd_major) * 100) + 0.5); // substitue for rint
				}
		...
		case wxSOCKET_LOST: {
			//          wxSocketError e = m_sock->LastError();          // this produces
...
//--------------Simulator
#if (0) // FIXME: no, there has to be another solution...
	{
		if (m_pShareMutex)
		...
		ll_gc_ll(ThreadPositionData.kLat, ThreadPositionData.kLon, kCog, dist, &pred_lat,
				 &pred_lon);
...
bool NMEAHandler::SendRouteToGPS(Route* pr, wxString& com_name, bool bsend_waypoints,
								 wxGauge* pProgress)
{
	bool ret_bool = false;
		...
		{ // Standard NMEA mode
			...
			if (snt.Sentence.Len() > max_length) {
				...
				if (g_GPS_Ident != _T("FurunoGP3X")) {
					...
					if ((pr->m_RouteNameString.IsNumber()) && (pr->m_RouteNameString.Len() <= 2)) {
...
OCP_GPSD_Thread::OCP_GPSD_Thread(NMEAHandler* Launcher, wxWindow* MessageTarget, wxString& ip_addr,
								 int api)
...
void* OCP_GPSD_Thread::Entry()
{
	...
	while ((not_done) && (m_launcher->m_Thread_run_flag > 0)) {
		...
		} else if (s_fn_gps_waiting(m_pgps_data, 1000000)) // 1 sec
		{
			...
			if (g_bDebugGPSD)
				printf("  Poll/Read Set: %0X\n", (unsigned int)m_pgps_data->set);

			if (!(m_pgps_data->set
				  & m_PACKET_SET)) // this is what sometimes happens when gpsd is killed or dies
				b_try_reopen = true;
			...
			if (m_pgps_data->set & m_SPEED_SET) {
				...
				if (!wxIsNaN(m_pfix->speed))
					ThreadPositionData.kSog = m_pfix->speed * 3600.
											  / 1852.; // convert from m/s to knots
			}

			if (m_pgps_data->set & m_SATELLITE_SET) {
				if (g_bDebugGPSD)
					printf("  SATELLITE_SET  %d\n", *m_psats_viz);
				ThreadPositionData.nSats = *m_psats_viz;
			}

			if ((m_libgps_api != 19)
				&& (m_pgps_data->set & m_ERROR_SET)) // not (easily) available on API 19
			{
				if (g_bDebugGPSD) {
			...
			if (m_bgps_present) // GPS must be online
			{
				...
				if (g_bDebugGPSD)
					printf("  STATUS_SET: %d status %d\n", (m_pgps_data->set & m_STATUS_SET) != 0,
						   *m_pstatus);
...
bool OCP_GPSD_Thread::OpenLibrary(void)
{
	...
	if (19 == m_libgps_api) {
		...
		if (!m_pgps_data) {
			if (g_bDebugGPSD)
				printf("  Could not (re)open libgps API 19\n");
			return false;
		}
...
//    Once more, gpsd architects do it again......
		...
		int result_open
			= s_fn_gps_open(m_GPSD_data_ip.char_str(), (char*)DEFAULT_GPSD_PORT, m_pgps_data);

		if (result_open != 0) {
			if (g_bDebugGPSD)
				printf("  Could not (re)open libgps API 20\n");
			m_pgps_data = NULL;
			return false;
		}
		m_pfix = &(m_pgps_data->fix);
...
OCP_NMEA_Thread::OCP_NMEA_Thread(NMEAHandler* Launcher, wxWindow* MessageTarget, wxMutex* pMutex,
								 wxMutex* pPortMutex, const wxString& PortName,
								 ComPortManager* pComMan, const wxString& strBaudRate, bool bGarmin)
{
	m_launcher = Launcher; // This thread's immediate "parent"
			...
			if (nl_found) {
				...
				while ((*tptr != 0x0a) && (tptr != put_ptr)) {
					*ptmpbuf++ = *tptr++;
					...
					wxASSERT_MSG((ptmpbuf - temp_buf) < RX_BUFFER_SIZE,
...
{
public:
	GARMIN_IO_Thread(DeviceMonitorWindow* parent, wxEvtHandler* MessageTarget, wxMutex* pMutex,
					 HANDLE device_handle, size_t max_tx_size);
...
GARMIN_IO_Thread::GARMIN_IO_Thread(DeviceMonitorWindow* parent, wxEvtHandler* MessageTarget,
								   wxMutex* pMutex, HANDLE device_handle, size_t max_tx_size)
{
...
void* GARMIN_IO_Thread::Entry()
{
	...
	while (m_parent->m_Thread_run_flag > 0) {
		...
		if (iresp.gusb_pkt.pkt_id[0] == GUSB_RESPONSE_PVT) // PVT Data Record
		{
			...
			/* speed over ground */
			ThreadPositionData.kSog = sqrt(ppvt->east * ppvt->east + ppvt->north * ppvt->north)
									  * 3.6 / 1.852;
...
top:
	...
	if ((rv > 0) && (ibuf->gusb_pkt.pkt_id[0] == GUSB_REQUEST_BULK)) {
		...
		goto top; // FIXME: implementation of a loop with goto, FFS...
	}
	/*
...
GarminEvent::GarminEvent(wxEventType commandType, int id) : wxEvent(id, commandType)
{
}
...
DeviceMonitorWindow::DeviceMonitorWindow(NMEAHandler* parent, wxWindow* MessageTarget,
										 wxMutex* pMutex)
	: wxWindow(parent->GetParentFrame(), wxID_ANY, wxPoint(40, 40), wxSize(5, 5), wxSIMPLE_BORDER)
{
	...
	// (wxObjectEventFunction)(wxEventFunction)&DeviceMonitorWindow::OnEvtGarmin);
			...
			{
				{
					...
					//    Start the pump
					m_io_thread = new GARMIN_IO_Thread(this, m_pMainEventHandler, m_pShareMutex,
													   m_usb_handle, m_max_tx_size);
					m_Thread_run_flag = 1;
					m_io_thread->Run();
				}
			}
...
HANDLE DeviceMonitorWindow::garmin_usb_start()
{
	...
	if ((m_usb_handle == INVALID_HANDLE_VALUE) || (m_usb_handle == 0)) {
		...
		filterData.dbch_handle
			= m_usb_handle; // file handle used in call to RegisterDeviceNotification
		filterData.dbch_hdevnotify = 0; // returned from RegisterDeviceNotification

		HDEVNOTIFY m_hDevNotify
			= RegisterDeviceNotification(GetHWND(), &filterData, DEVICE_NOTIFY_WINDOW_HANDLE);
...

{
	...
	if ((!m_pdata_ap_port_string->IsEmpty())
		&& (!m_pdata_ap_port_string->IsSameAs(_T("None"), false))) {
