...
bool S57Obj::IsUsefulAttribute(char* buf)
{
	...
	// GRUP
	// --------------------------------------------------------------------
	else if (!strncmp(buf, "  GRUP", 6)) return false;

	// --------------------------------------------------------------------
	// OBJL
	// --------------------------------------------------------------------
	else if (!strncmp(buf, "  OBJL", 6)) return false;

	// --------------------------------------------------------------------
	// RVER
	// --------------------------------------------------------------------
	else if (!strncmp(buf, "  RVER", 6)) return false;

	// --------------------------------------------------------------------
	// AGEN
	// --------------------------------------------------------------------
	else if (!strncmp(buf, "  AGEN", 6)) return false;

	// --------------------------------------------------------------------
	// FIDN
	// --------------------------------------------------------------------
	else if (!strncmp(buf, "  FIDN", 6)) return false;

	// --------------------------------------------------------------------
	// FIDS
	// --------------------------------------------------------------------
	else if (!strncmp(buf, "  FIDS", 6)) return false;

	// UnPresent data
	else if (strstr(buf, "(null)")) return false;
		...
		switch (cs) {
			...
			case global::GLOBAL_COLOR_SCHEME_NIGHT: {
				if (NULL == m_pDIBThumbDim) {
					...
					// factors are completely subjective
					wxImage gimg = img.ConvertToGreyscale(0.1, 0.1, 0.1);
#else
					wxImage gimg = img;
#endif
...
ThumbData* s57chart::GetThumbData(int, int, const geo::Position& pos)
{
								...
								RenderTypeEnum WXUNUSED(option), bool force_new_view)
{
	...
	if (bNewVP && (NULL != pDIB) && !rgn_last.IsEmpty()) {
		...
		while (upd.HaveRects()) {
			...
			geo::Position p0 = geo::fromSM(temp_easting_ul, temp_northing_ul, reference_point);
			...
			geo::Position p1 = geo::fromSM(temp_easting_lr, temp_northing_lr, reference_point);
...
#else
	delete prender_image; // the image owns the data and so will free it in due course
#endif
...
InitReturn s57chart::Init(const wxString& name, ChartInitFlag flags)
{
	...
	reference_point = geo::Position((m_FullExtent.NLAT + m_FullExtent.SLAT) / 2.0,
									(m_FullExtent.WLON + m_FullExtent.ELON) / 2.0);
...
InitReturn s57chart::FindOrCreateSenc(const wxString& name)
{
	...
	if (m_SENCFileName.FileExists()) {
		...
		if (f.Open(m_SENCFileName.GetFullPath())) {
			if (f.Length() == 0) {
			...
			} else { // file exists, non-zero so check for new updates
				...
				while (!dun) {
					if (my_fgets(pbuf, 256, *pfpx) == 0) {
						...
						if (token.IsSameAs(_T("UPDT"), true)) {
							...
							if (!ModTime000.ParseFormat(str, _T("%Y%m%d")))
				...
				// and all updates (.001, .002, etc.)  are subsumed.
					...
					// Make two simple tests to see if the .000 file is "newer" than the SENC file
					// representation
					// These tests may be redundant, since the DSID:EDTN test above should catch new
					// base files
...
WX_DEFINE_ARRAY(float*, MyFloatPtrArray); // FIXME: replace this, use std::vector
...
bool s57chart::CreateHeaderDataFromENC(void)
{
	...
	if (m_nCOVREntries == 1) {
		...
		*m_pCOVRTable = (float*)malloc(pAuxCntArray->Item(0) * 2 * sizeof(float));
		...
		// strange case, found no CATCOV=1 M_COVR objects
	...
	if (0 == m_nCOVREntries) { // fallback
		wxLogMessage(_T("   ENC contains no M_COVR features:  ") + m_FullPath);
		wxLogMessage(_T("   Calculating Chart Extents as fallback."));
		...
		if (pENCReader->GetExtent(&Env, true) == OGRERR_NONE) {
			...
			*pfe++ = LonMin;
			...
			wxLogMessage(_T("   Cannot calculate Extents for ENC:  ") + m_FullPath);
...
bool s57chart::CreateHeaderDataFromSENC(void)
{
	...
	if (!m_SENCFileName.FileExists()) {
		wxLogMessage(_T("   Cannot open SENC file ") + m_SENCFileName.GetFullPath());
		return 1;
	}
...
ListOfS57Obj* s57chart::GetAssociatedObjects(S57Obj* obj)
{
	...
	geo::Position pos
		= geo::fromSM((obj->x * obj->x_rate) + obj->x_origin,
					  (obj->y * obj->y_rate) + obj->y_origin, reference_point);
...
int s57chart::GetUpdateFileArray(const wxFileName file000, wxArrayString* UpFiles)
{
	...
	while (cont) {
		...
		if (ext.ToLong(&tmp) && (file.GetName() == file000.GetName())) {
			...
			if (buffer.data() && !filename.IsSameAs(_T("CATALOG.031"), false)) {
				// don't process catalogs
				...
				if (!poModule->Open(FileToAdd.mb_str())) {
					...
					// n.b. assumes that the required fields will be in Record 0....  Is this always
					// true?
					...
					if (pr) {
						...
						wxString msg(_T("   s57chart::BuildS57File  DDFRecord 0 does not contain ")
				...
				if ((!umdate.IsEarlierThan(m_date000))
					&& (umedtn.IsSameAs(m_edtn000))) // Note polarity on Date compare....
	...
	if (UpFiles->size()) {
		...
		if (b_copyfiles) {
			...
			for (int iff = 0; iff < retval + 1; iff++) {
				...
				ufile.SetExt(wxString::Format(_T("%03d"), iff));
		...
		lastfile.SetExt(wxString::Format(_T("%03d"), retval));
...
bool s57chart::GetBaseFileAttr(wxFileName fn)
{
	...
	if (!poModule->Open(FullPath000.mb_str())) {
		wxLogMessage(_T("   s57chart::BuildS57File  Unable to open ") + FullPath000);
		return false;
	}
	...
	if (!m_nGeoRecords) {
		wxLogMessage(_T("   s57chart::BuildS57File  DDFRecord 0 does not contain DSSI:NOGR "));
	...
	if (u) {
		...
		wxLogMessage(_T("   s57chart::BuildS57File  DDFRecord 0 does not contain DSID:ISDT "));
		date000 = _T("20000101"); // backstop, very early, so any new files will update?
	}
	m_date000.ParseFormat(date000, _T("%Y%m%d"));
	if (!m_date000.IsValid())
		m_date000.ParseFormat(_T("20000101"), _T("%Y%m%d"));
	...
	if (u) {
		...
		wxLogMessage(_T("   s57chart::BuildS57File  DDFRecord 0 does not contain DSID:EDTN "));
	...
	if (!m_native_scale) {
		wxLogMessage(_T("   s57chart::BuildS57File  ENC not contain DSPM:CSCL "));
...
int s57chart::BuildSENCFile(const wxString& FullPath000, const wxString& SENCFileName)
{
	...
	wxLogMessage(_T("Building SENC file for ") + FullPath000 + _T(" to ") + SENCFileName);
	...
	if (fps57 == NULL) {
		wxLogMessage(_T("   s57chart::BuildS57File  Unable to create temp SENC file ") + tfn.GetFullPath());
		return 0;
	}
	...
	s_ProgDialog
		= new wxProgressDialog(Title, Message, m_nGeoRecords, NULL,
	...
	int open_return = poS57DS->Open(m_tmpup_array->Item(0).mb_str(), true, NULL);
	...
	while (bcont) {
		...
		//  CE_Fatal happened in GDAL library
		...
		if (objectDef != NULL) {
			...
			if (geoType != wkbUnknown) {
				// Write only if has wkbGeometry
...
abort_point:
	...
	if (bcont) {
		...
		if (!cpok) {
			wxLogMessage(_T("   Cannot copy temporary SENC file ") + tmp_file + _T(" to ")
						 + SENCfile.GetFullPath());
			ret_code = BUILD_SENC_NOK_RETRY;
		} else
...
int s57chart::BuildRAZFromSENCFile(const wxString& FullPath)
{
	...
	if (!SENCFileName.FileExists()) {
		wxLogMessage(_T("   Cannot open SENC file ") + SENCFileName.GetFullPath());
...
int s57chart::_insertRules(S57Obj* obj, LUPrec* LUP, s57chart* pOwner)
{
	...
	// insert rules
	rzRules = (ObjRazRules*)malloc(sizeof(ObjRazRules)); // FIXME: replace malloc
	rzRules->obj = obj;
	...
	for (int i = 0; i < PRIO_NUM; ++i) {
		...
		if ((razRules[i][0]) && (NULL == razRules[i][1])) {
			...
			while (top != NULL) {
				...
				if (LUP) {
					// A POINT object can only appear in two places in the table, SIMPLIFIED or
					// PAPER_CHART
...
#define MAX_HDR_LINE 400
			...
			if ((iField == 1) || (iField > 7)) {
				...
				if (strlen(pAttrVal)) {
					...
					if ((0 == strncmp("NOBJNM", pAttrName, 6))
						...
						if (poReader->GetNall() == 2) {
							// ENC is using UCS-2 / UTF-16 encoding
							wxMBConvUTF16 conv;
						...
						} else if (poReader->GetNall() == 1) {
							// ENC is using Lex level 1 (ISO 8859_1) encoding
					...
					if (wxAttrValue.IsEmpty()) {
						// Attempt different conversions to accomodate different language encodings
						// in the original ENC files.
	...
	if (wkbPoint == pGeo->getGeometryType()) {
		...
		if (10 != nqual) {
			// only add attribute if nQual is not "precisely known"
	...
	if (mode == 1) {
		sprintf(line, "  %s %g %g\n", pGeo->getGeometryName(), reference_point.lat(),
				reference_point.lon());
		sheader += wxString(line, wxConvUTF8);
	}
	...
	if ((pGeo != NULL)) {
		...
		switch (gType) {
			case wkbLineString: {
				// data will be 4 byte float, not double and bbox limits are tacked on end
				sb_len = ((wkb_len - 9) / 2) + 9 + 16;
				fprintf(fpOut, "  %d\n", sb_len);
			...
			case wkbMultiLineString:
				wxLogMessage(_T("   Warning: Unimplemented SENC wkbMultiLineString record in file ")
			...
			case wkbPoint: {
				...
				// data will be 4 byte float, not double and skipping nQual
				sb_len = ((wkb_len - (5 + nq_len)) / 2) + 5;
			...
			case wkbMultiPoint25D:
				...
				// GTYPE and count, points as floats and trailing bbox
				sb_len = (9 + nPoints * 3 * sizeof(float)) + 16;
				fprintf(fpOut, "  %d\n", sb_len);
			...
			// All others
			default:
				wxLogMessage(_T("   Warning: Unimplemented ogr geotype record in file ")
							 + m_SENCFileName.GetFullPath());
...
wxString s57chart::GetAttributeDecode(wxString& att, int ival)
{
	...
	if (!wxFileName::FileExists(file)) {
		wxLogMessage(_T("   Could not open ") + file);
		return ret_val;
	}
	...
	if (!wxFileName::FileExists(ei_file)) {
		wxLogMessage(_T("   Could not open ") + ei_file);
		return ret_val;
	}
...
bool s57chart::InitFromSENCMinimal(const wxString& FullPath)
{
	...
	if (f.Open(S57FileName.GetFullPath())) {
		if (f.Length() == 0) {
			...
			// file exists, non-zero so check for new updates
				...
				if (val_str.ToLong(&ival)) {
					if (0 == ival)
						...
						if (!decode_val.IsEmpty()) {
							value = decode_val + wxString::Format(_T("(%d)"), (int)ival);
						} else {
							value = wxString::Format(_T("%d"), (int)ival);
						}
					}
								  ...
								  std::vector<s57Sector_t>& sectorlegs)
{
	...
	if (sectorlegs.size() > 0) {
		...
		for (unsigned int i = 0; i < sectorlegs.size(); i++) {
			...
			double step = 3.0;
			while ((step < 15) && ((rangePx * sin(step * M_PI / 180.)) < 10))
			...
			for (unsigned int j = 0; j < sectorangles.size(); j++) {
				...
				if (sectorangles[j] == sec1)
					haveAngle1 = true;
				if (sectorangles[j] == sec2)
					haveAngle2 = true;
			}
