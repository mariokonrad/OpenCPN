...
wxString AIS_Target_Data::BuildQueryResult(void)
{
	...
	wxString tcpaStr;
	if (bCPA_Valid)
		tcpaStr << _T("/<b> ") << _("in") << _T("<b>") << FormatTimeAdaptive((int)(TCPA * 60.));

	if (bCPA_Valid) {
		html << vertSpacer << rowStart << _("CPA") << rowEnd << rowStartH << _T("<b>")
			 << cc1->FormatDistanceAdaptive(CPA) << tcpaStr << rowEnd;
	}
